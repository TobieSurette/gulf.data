...
read.scobs <- function(year, file, path,  zone, cfvn, type = "sea", trip.number,
                          database = "gap", username = "4R_GAP", password, source = "oracle", ...){

   # 'x' is a file name:
   if (!missing(file)) if (is.character(file)) source <- "ascii"

   # Parse 'source' argument:
   source <- match.arg(tolower(source), c("r", "ascii", "csv", "oracle"))

   # Read R or csv file:
   if (source %in% c("r", "csv")){
      path <- scobs.path.str(...)
      spath <- strsplit(path, "/")
      zone.str <- gsub("Zone ", "", unlist(lapply(spath, function(x) x[grep("Zone", x)])))
      zone.str[zone.str %in% c("E", "F")] <- paste0("12", zone.str[zone.str %in% c("E", "F")])
      year.str <- gsub("Fishing Year ", "", unlist(lapply(spath, function(x) x[grep("Fishing Year", x)])))
      type.str <- gsub(" Sample", "", unlist(lapply(spath, function(x) x[length(x)])))
      type.str[type.str == "Sea"] <- "S"
      type.str[type.str == "Port"] <- "P"

      path <- paste0(unlist(lapply(strsplit(path, "Raw Data"), function(x) x[1])), "Databases")
      if (source == "r") files <- paste0(path, "/", zone.str, type.str, year.str, ".Rdata")
      if (source == "csv") files <- paste0(path, "/", zone.str, type.str, year.str, ".csv")
      files <- files[file.exists(files)]
      if (length(files)){
         res <- NULL
         for (i in 1:length(files)){
            cat(paste0("Reading : '", files[i], "'\n"))
            if (source == "r") load(file = files[i])
            if (source == "csv"){
   ...
   # Read 'GAP' Oracle database:
   if (source %in% c("oracle")){
      # Open Oracle channel:
      if (!missing(password)) channel <- RODBC::odbcConnect(dsn = "gap", uid =  "4R_GAP", pwd = password, believeNRows = FALSE) else stop("'password' must be specified.")
     ...
     query <- "select  -- decode(a.NO_SEQ_CASIER, 99, 'P', 'S') || mod(extract(year from a.dat_deb_acti_pech), 100) || a.NO_SORTIE filename
      ...
      # Execute query:
      z <- oracle.query(channel, query, as.is = TRUE)
      ...
      # Set 'scobs' attributes:
      class(z) <- class(scobs())
      attr(z, "converted") <- TRUE
      key(z) <- key(scobs())
      attr(z, "format") <- fmt.scobs()
   ...
   if (source == "ascii"){
      ...
      # Read file(s):
      x <- read.fortran(file = file, format = f$format)
      names(x) <- f$name
